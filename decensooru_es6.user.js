...
decensooru: {
  ...
  const Decensor = {
    newThumbs({ target }) {
      if (target.tagName !== "IMG" || target.parentNode.tagName !== "PICTURE" || target.src.includes("raikou4")) return;
    ...
    post() {
      return _asyncToGenerator(function* () {
        const details = yield Decensor.getDetails();
        if (!details) return;
    ...
    pullBatch() {
      return _asyncToGenerator(function* () {
        fetcher: try {
          ...
          const x = (yield $.xhr(DataBase.batches + batchNumber)).target;
          switch (x.status) {
            case 404:
              DataBase.notDone = false;break fetcher;
            case 200:case 304:
              break;
            default:
